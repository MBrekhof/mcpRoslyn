...
| `get_compilation_errors` | **opt-in** (`includeAnalyzers: true`) | ~45 ms, 548 diagnostics; **~340 ms since DIAG-003** (2026-09-13, 503 diagnostics), because BPG.Data's EF Core suppressor makes the suppressor pass run | ~2.4 s (~52x), 1216 diagnostics; 2026-09-13: ~2.0 s, 1135 |
...
`DiagnosticSuppressor`s run through `RoslynHelpers.ApplySuppressorsAsync`. When a loaded suppressor supports suppressing a compiler diagnostic ID that is present, a whole-compilation pass containing only the project's suppressors runs with the same analyzer options and suppressed diagnostics excluded; with no applicable suppressor it doesn't run. `get_compilation_errors` applies them **always**, because a suppressed diagnostic is not one the build reports: EF Core's CS8618 on `DbSet<T>` properties became a false error under `WarningsAsErrors=Nullable` (DIAG-003). `get_document_diagnostics` applies them only with analyzers on, replacing the file's compiler diagnostics with that pass's results for its syntax tree. Its `includeAnalyzers: false` stays the explicit fast compiler-only path, which a whole-compilation pass would defeat.
